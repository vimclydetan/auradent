...
<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <div>
            <h3 class="text-xl font-bold text-slate-800 tracking-tight italic uppercase flex items-center gap-2">
                <i class="fas fa-calendar-alt text-blue-600"></i>
                Appointment Calendar
            </h3>
            <p class="text-xs text-slate-500 font-bold italic uppercase mt-1">
                Pending, Confirmed and Cancelled schedules only
            </p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all">
        ...
        <div class="p-6 space-y-5">
            ...
            <div class="pt-4 border-t border-slate-100">
                <a href="<?= base_url('receptionist/appointments') ?>" class="block w-full text-center bg-slate-100 hover:bg-slate-200 text-slate-600 py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all">
...
<script src="/fullcalendar/core.js"></script>
<script src="/fullcalendar/daygrid.js"></script>
<script src="/fullcalendar/timegrid.js"></script>
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            height: 750,
            events: '<?= base_url('receptionist/calendar/events') ?>',

            // CUSTOM EVENT DISPLAY
            eventContent: function(arg) {
                // Format time to 12h (h:mm A)
                let timeStr = arg.event.start.toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit',
                    hour12: true
                });
    ...
    function openViewModal(event) {
        ...
        document.getElementById('det_date').innerText = event.start.toLocaleDateString('en-US', {
            month: 'long',
            day: 'numeric',
            year: 'numeric'
        });
        ...
        if (status === 'Pending') {
            ...
            header.className = 'p-5 flex justify-between items-center text-white bg-red-600';
            statusEl.className = 'px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-red-100 text-red-600';
        }
